...
function LibraryContent() {
  ...
  const [isAssigningBoard, setIsAssigningBoard] = useState<string | null>(null);

  useEffect(() => {
    fetchCards();
    
    const handleCardSaved = () => fetchCards();
    window.addEventListener("card-saved", handleCardSaved);
    return () => window.removeEventListener("card-saved", handleCardSaved);
  }, [fetchCards]);
    ...
    try {
      ...
      if (res.ok) {
        setBoards(prev => prev.map(b => 
          b.id === boardId ? { ...b, card_count: (b.card_count || 0) + 1 } : b
        ));
        toast.success("Added to board!");
        setTimeout(() => setAddToBoardCard(null), 500);
      } else {
        const data = await res.json();
              ...
              <div className="space-y-2">
                {boards.map((board) => (
                  <button
                    ...
                    disabled={isAssigningBoard !== null}
                    className="flex w-full items-center gap-3 rounded-lg border border-border p-3 text-left transition-colors hover:bg-accent cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
                  >
